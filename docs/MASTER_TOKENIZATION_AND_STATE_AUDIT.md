...
│ فاز ۴   │ اتاق فرمان، تب‌ها و آمارها (Dashboard)    │  آماده اجرا      │
│ فاز ۵   │ مودال‌ها، تنظیمات و اسکن سراسری مخزن      │  در نوبت          │
└─────────┴───────────────────────────────────────────┴───────────────────┘
```
...
## ۵. پیش‌نمایش و چک‌لیست فازهای آتی (Phases 4 to 5 Checklists)

### فاز ۴: اتاق فرمان و داشبورد (`CycleDashboardView`) [گام بعدی]
- [ ] نوار زیرتب‌ها (`ResponsiveSubTabBar`) و تطبیق با استیت‌های فاز ۱
- [ ] کارت‌های تثلیث مقدس دیسیپلین (استریک، روز استاندارد، امتیاز کل)
- [ ] کارت‌های آمار ثانویه بر اساس اصل ضد-رنگین‌کمان (Anti-Rainbow)
- [ ] ویجت‌های تحلیلی، نمودارهای روند و ماتریس وفاداری

### فاز ۵: لایه‌های غوطه‌ور، مودال‌ها، تنظیمات و اسکن سراسری
- [ ] مودال کالبدشکافی خطا (`AutopsyModal`)
- [ ] مودال تسویه بدهی (`DebtModal`)
- [ ] مودال پرداخت و ارتقای VIP (`PaymentModal`)
- [ ] صفحه تنظیمات و پروفایل (`ProfileSettingsView`)
- [ ] اسکن خودکار کل فایل‌های سورس برای یافتن هرگونه متغیر هاردکدشده یا خطای نگارشی کلاس‌ها
