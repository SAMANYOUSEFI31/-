...
- **پالت رنگ‌های سمانتیک انحصاری (Semantic Palette):**
  ...
  - Tactical Orange / Flame (زنجیره خالص پیوسته و بدون خدشه): `#F97316` و `#FB923C` (رنگ رسمی و انحصاری استریک)
...
- **نشانگر شعله استریک (Pure Streak):**
  - اتصال به توکن‌های سمانتیک استریک پایدار: `bg-orange-subtle text-orange radius-component`. عدم تغییر رنگ با تم انتخابی کاربر (حفظ استواری نماد استریک).
...
- **بج‌های تله‌متری و سرزندگی زنجیره (Streak Vitality):**
  ...
  - بج حیاتی استریک: استفاده از `entity-status-badge` با رنگ‌های سمانتیک استاندارد (`text-orange bg-orange-subtle` برای روز استاندارد، `text-blue bg-blue-subtle` برای فریز موجه، و `text-debt bg-debt-subtle` برای شکست).
...
- **کارت ۱: زنجیره فعال (Pure Streak):**
  - متصل به آیکون `Flame` در کانتینر `bg-orange-subtle`، عدد برجسته با رنگ نارنجی سمانتیک `text-orange` (ثبات قطعی در تمام تم‌ها).
...
- **رکوردهای تاریخی سه‌گانه:**
  - تطبیق ۱۰۰٪ پالت رنگ با ارکان تثلیث (نارنجی تاکتیکی برای استریک، زمرد برای روزهای استاندارد، کهربا برای امتیاز کل).
...
### ۶.۴. وضعیت نهایی ممیزی توکن‌ها و مدیریت بدهی فنی (Token Status & Known Debt)
- نماهای اصلی و فرآیندهای حیاتی کاربر (کارزار نبرد، اتاق فرمان، ناوبری، مودال‌های اصلی و ارکان تثلیث دیسیپلین) به طور کامل به توکن‌های سمانتیک CSS متغیر `tokens.css` متصل شده‌اند.
- **بدهی فنی شناخته‌شده (Known Technical Debt):** کامپوننت‌های نصب PWA (`PwaInstallBanner.tsx`, `IosInstallTip.tsx`) و بخش‌هایی از پنل مدیریت (`AdminView.tsx`) ممکن است تا زمان توکنایزیشن جامع دارای استایل‌های پالت خام تیل‌ویند (زینک/اسلیت/هگز) باشند که در چرخه‌های آتی بازآفرینی می‌شوند.
